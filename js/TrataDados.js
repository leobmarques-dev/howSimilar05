			...
			$.each(valorAtual, function(chave2, valorCampo){
                if(chave == 0){
                	if($.isArray(valorCampo[chave2])){
                		cabecalhosArray.push(valorCampo[chave2].toLowerCase());
                	} else{
                		cabecalhosArray.push(valorCampo.toLowerCase());
                	}
                } else {
                    valoresArray[chave].push(valorCampo.toLowerCase());
				...
				$.each(json, function(chave, valorAtual){
		            ...
		            for ( var key in valorAtual){
		                ...
		                if(valorAtual.hasOwnProperty(key)){
		                    ...
		                    if(key == "Modelo"){
		                        cabecalhosArray[chave] = valorAtual.Modelo.toLowerCase();
		                    } else{
		                        valoresArray[chave].push(valorAtual[key].toLowerCase())
